...
import { Bookmark, BookmarkCheck, Play, Pencil } from "lucide-react";
import { useState } from "react";
import { fetchAnimeDetail } from "@/lib/api";
import { mergeDetail } from "@/lib/overlay";
import { useHikariStore } from "@/lib/store";
import { displayTitle } from "@/lib/types";
import { formatLabel, genreLabel, scoreLabel, seasonLabel, statusLabel } from "@/lib/labels";
...
function AnimePage() {
  ...
  const episodeCount =
    seasons.reduce((n, s) => n + s.episodes.length, 0) || anime.episodesCount || 0;

  return (
    <article className="pb-8">
      <div className="relative -mx-4 h-48 overflow-hidden sm:-mx-6 sm:h-72">
        {(anime.banner || anime.cover) && (
          <img src={anime.banner || anime.cover} alt="" className="size-full object-cover" />
        )}
      ...
      <div className="relative z-10 -mt-20 flex flex-col gap-5 sm:-mt-24 sm:flex-row">
        <div className="mx-auto w-32 shrink-0 overflow-hidden rounded-lg bg-elevated shadow-[var(--shadow-border)] sm:mx-0 sm:w-40">
          {anime.cover ? (
            <img src={anime.cover} alt="" className="aspect-2/3 w-full object-cover" />
          ) : (
        ...
        <div className="flex-1 space-y-3 sm:pt-16">
          ...
          {anime.titles.native && (
            <p className="font-display text-sm text-muted">{anime.titles.native}</p>
          )}
          <div className="flex flex-wrap items-center gap-2">
            {anime.score != null && (
              <span className="text-sm font-medium tabular-nums text-score">{scoreLabel(anime.score)}</span>
            )}
            <span className="text-sm text-muted">{statusLabel(anime.status)}</span>
            {episodeCount > 0 && (
              <span className="text-sm text-muted">{episodeCount} episódios</span>
            )}
            {seasonLabel(anime.season, anime.year) && (
              <span className="text-sm text-muted">{seasonLabel(anime.season, anime.year)}</span>
            )}
          </div>
          <div className="flex flex-wrap gap-1.5">
            {anime.genres.map((g) => (
              <Link
                key={g}
                to="/search"
                search={{ q: "", genre: g, year: "", format: "", status: "", sort: "TRENDING_DESC" }}
              >
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button type="button" variant="outline" onClick={() => toggleList(anime)}>
              {inList ? <BookmarkCheck className="size-4" /> : <Bookmark className="size-4" />}
              {inList ? "Na lista" : "Minha Lista"}
            </Button>
            <Button asChild variant="ghost">
              <Link to="/admin/$id" params={{ id: localRecord?.id ?? "new" }} search={{ importId: anime.id }}>
      ...
      {anime.synopsis && (
        <section className="mt-8 max-w-3xl">
          <h2 className="text-[11px] tracking-[0.28em] text-muted uppercase">Sinopse</h2>
          <p className="mt-2 text-sm leading-relaxed text-muted">{anime.synopsis}</p>
        </section>
      )}

      {yt && (
        <section className="mt-8">
          <h2 className="text-[11px] tracking-[0.28em] text-muted uppercase">Trailer</h2>
        ...
        <section className="mt-10">
          ...
          <div className="mt-4 space-y-8">
            {seasons.map((season) => (
              <div key={season.id}>
                {seasons.length > 1 && (
                  <h3 className="mb-3 text-sm font-medium text-muted">{season.title}</h3>
                )}
                <EpisodeGrid
      ...
      {anime.studios.length > 0 && (
        <p className="mt-8 text-xs text-subtle">Estúdio: {anime.studios.join(", ")}</p>
      )}

      {anime.recommendations.length > 0 && (
        <section className="mt-10 space-y-3">
          <h2 className="font-display text-2xl tracking-tight">Relacionados</h2>
...
function EpisodeGrid({
  ...
  episodes: { id: string; number: number; title: string; thumbnail?: string; duration?: string }[];
  animeId: string;
  cover: string;
}) {
          ...
          <li key={ep.id}>
            ...
            >
              <div className="h-16 w-28 shrink-0 overflow-hidden rounded-md bg-elevated">
                {ep.thumbnail || cover ? (
                  <img src={ep.thumbnail || cover} alt="" className="size-full object-cover" />
                ) : null}
              </div>
              <div className="min-w-0 py-0.5">
                <p className="text-[11px] tabular-nums text-subtle">Ep. {ep.number}</p>
                <p className="truncate text-sm text-fg">{ep.title}</p>
                {ep.duration && <p className="text-xs text-subtle">{ep.duration}</p>}
              </div>
            </Link>
          </li>
        ))}
