...
function SearchPage() {
  ...
  const items = useMemo(() => {
    ...
    const localHits = overlayList([], locals).filter((a) => {
      if (q && !`${a.titles.romaji} ${a.titles.english} ${a.titles.native}`.toLowerCase().includes(q))
        return false;
  ...
  return (
    <div className="space-y-6 pt-6">
      <header className="space-y-1">
        <p className="text-[11px] tracking-[0.28em] text-muted uppercase">Catálogo</p>
        <h1 className="font-display text-3xl tracking-tight">Buscar</h1>
      </header>
      ...
      <div className={filtersOpen ? "grid gap-3 sm:grid-cols-2 lg:grid-cols-4" : "hidden lg:grid lg:grid-cols-4 lg:gap-3"}>
        <Field label="Gênero">
          <NativeSelect value={search.genre ?? ""} onChange={(e) => apply({ genre: e.target.value || undefined })}>
        ...
        <Field label="Ano">
          <NativeSelect value={search.year ?? ""} onChange={(e) => apply({ year: e.target.value || undefined })}>
        ...
        <Field label="Formato">
          <NativeSelect value={search.format ?? ""} onChange={(e) => apply({ format: e.target.value || undefined })}>
            <option value="">Todos</option>
            {Object.entries(FORMAT_PT)
              .filter(([k]) => ["TV", "MOVIE", "OVA", "ONA", "SPECIAL", "TV_SHORT"].includes(k))
              .map(([k, v]) => (
        ...
        <Field label="Status">
          <NativeSelect value={search.status ?? ""} onChange={(e) => apply({ status: e.target.value || undefined })}>
            <option value="">Todos</option>
        ...
        <Field label="Ordenar" className="lg:col-span-2">
          <NativeSelect value={search.sort ?? "TRENDING_DESC"} onChange={(e) => apply({ sort: e.target.value || undefined })}>
            {SORT_OPTIONS.map((o) => (
      ...
      {items.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <p className="mt-2 text-sm text-muted">Tente outro título ou limpe os filtros.</p>
          <Link to="/search" className="mt-4 inline-block text-sm text-fg underline">
            Limpar busca
          </Link>
        </div>
      ) : (
