...
type JikanAnime = {
  ...
  trailer?: { youtube_id?: string | null; images?: { maximum_image_url?: string } };
  studios?: { name: string }[];
...
async function jikanEpisodes(malId: number): Promise<Season[]> {
  ...
  while (hasNext && page <= 8) {
    ...
    const json = await jikanFetch<EpPage>(`/anime/${malId}/episodes?page=${page}`);
  ...
  return [{ id: `mal-s1-${malId}`, number: 1, title: "Temporada 1", episodes }];
}

function streamingFromAni(media: AniMedia): StreamingLink[] {
...
function mapAniFull(media: AniMedia, seasons: Season[]): Anime {
  const slim = mapAniSlim(media);
  return {
    ...slim,
    duration: media.duration ?? null,
    studios: (media.studios?.nodes ?? []).map((n) => n.name),
    nextEpisode: media.nextAiringEpisode ?? undefined,
    streamingEpisodes: streamingFromAni(media),
    seasons,
    recommendations: (media.recommendations?.nodes ?? [])
      .map((n) => n.mediaRecommendation)
      .filter((m): m is AniMedia => Boolean(m?.id))
      .slice(0, 12)
      .map(mapAniSlim),
  };
}
...
async function fetchHomeFromAni(): Promise<HomeCatalog> {
  ...
  const data = await anilistGraphQL<{
    ...
    popular: { media: AniMedia[] };
    top: { media: AniMedia[] };
    season: { media: AniMedia[] };
    genres: string[];
  }>(`
...
async function fetchHomeFromJikan(): Promise<HomeCatalog> {
  const { season, year } = currentAnimeSeason();
  type List = { data?: JikanAnime[] };
  const [now, top, popular] = await Promise.all([
    jikanFetch<List>("/seasons/now?limit=18"),
    jikanFetch<List>("/top/anime?filter=bypopularity&limit=18"),
    jikanFetch<List>("/top/anime?limit=18"),
  ]);
  const seasonItems = (now.data ?? []).map(mapJikanSlim);
  const popularItems = (top.data ?? []).map(mapJikanSlim);
  const topItems = (popular.data ?? []).map(mapJikanSlim);
  return {
...
export const fetchHomeCatalog = createServerFn({ method: "GET" }).handler(async () => {
  const key = "home";
  const cached = fromCache<HomeCatalog>(key);
...
async function searchAni(params: SearchParams): Promise<SearchResult> {
  const page = params.page ?? 1;
  const sort = params.sort || (params.q ? "SEARCH_MATCH" : "TRENDING_DESC");
  const year = params.year ? Number(params.year) : undefined;
  const data = await anilistGraphQL<{
    Page: { pageInfo: { hasNextPage: boolean }; media: AniMedia[] };
  }>(
    `
    query Search(
...
async function searchJikan(params: SearchParams): Promise<SearchResult> {
  const page = params.page ?? 1;
...
export const searchCatalog = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    const cached = fromCache<SearchResult>(key);
    if (cached) return cached;
    try {
...
export const fetchAnimeDetail = createServerFn({ method: "GET" })
  .validator(idSchema)
  .handler(async ({ data }) => {
    const { id } = data;
    if (id.startsWith("local-")) return null;
    const key = `detail:${id}`;
    const cached = fromCache<Anime>(key);
    if (cached) return cached;

    if (id.startsWith("mal-")) {
      const malId = Number(id.replace("mal-", ""));
      const json = await jikanFetch<{ data: JikanAnime }>(`/anime/${malId}/full`);
    ...
    try {
      const result = await anilistGraphQL<{ Media: AniMedia | null }>(
        `
        query Detail($id: Int) {
      ...
      if (!seasons.length && (media.streamingEpisodes?.length || media.episodes)) {
        const fromStream: Episode[] = (media.streamingEpisodes ?? []).map((e, i) => ({
        ...
        const episodes =
          fromStream.length >= count
            ? fromStream
            : Array.from({ length: count }, (_, i) => {
                const found = fromStream[i];
                return (
                  found ?? {
                    id: `ep-${media.id}-${i + 1}`,
                    number: i + 1,
        ...
        seasons = [{ id: `s1-${media.id}`, number: 1, title: "Temporada 1", episodes }];
      }
      return toCache(key, mapAniFull(media, seasons));
    } catch {
      // Jikan as secondary: treat numeric as MAL id if AniList failed
      try {
        const json = await jikanFetch<{ data: JikanAnime }>(`/anime/${anilistId}/full`);
        const slim = mapJikanSlim(json.data);
        let seasons: Season[] = [];
        try {
...
const browseSchema = z.object({
  section: z.enum(["popular", "season", "top", "trending"]),
  page: z.number().optional(),
});

export const fetchBrowse = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }) => {
    ...
    try {
      ...
      `,
        isSeason
          ? { page, sort: [sortMap.season], season, year }
          : { page, sort: [sortMap[data.section]] },
      );
      return toCache(key, {
